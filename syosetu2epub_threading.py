...
class Novel:
    ...
    def genBook(self):
        ...
        with open(os.path.join(__location__, 'files/content.opf'), encoding="utf-8") as t:
            ...
            finalOutput = template.substitute(IDTAG=self.seriesCode, TITLETAG=self.title, AUTHORTAG=authorName, TIMESTAMPTAG=datetime.now(pytz.utc).isoformat().split('.', 1)[0] + 'Z', CHAPTERSTAG=chapterList, SPINETAG=chapterListAgain, IMAGESTAG=imgList)
            oebpsDir = os.path.join(self.tempDir.name, self.title, "OEBPS")
